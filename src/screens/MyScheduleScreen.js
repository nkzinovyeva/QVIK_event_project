...
export default function MyScheduleScreen({ navigation }) {
      ...
      header: () => (
        <AppHeader
          ...
          subTitle={
            ...
            setupData.venue +
            ", " +
            (moment(setupData.startDate).format("MMM Do") +
              " - " +
              moment(setupData.endDate).format("Do YYYY"))
          }
          leftButton={false}
          rightButton={false}
          clickableTag={false}
  ...
  const Event = ({ item }) => {
    ...
    if (item.active == false) {
      ...
      (item.startDate == nowDate && item.startTime > nowTime)
    ) {
      status = "passed";
    }
    ...
    return (
      <TouchableOpacity
        onPress={() => navigation.navigate("Event", item.eventId)}
      >
        <AppList
          leftIcon={true}
  ...
  return (
    <SafeAreaView style={styles.container}>
      {favourites.length === 0 ? (
        <Text style={styles.replacementText}>Add events to favourite list</Text>
      ) : (
        <FlatList
          data={favourites}
