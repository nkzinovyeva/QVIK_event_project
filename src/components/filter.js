...
export default AppFilter = (props) => {
  ...
  const { eventTags, events, setupData, restsTags, restaurants } = useSelector(
    (state) => state.eventsReducer
  );
  const dispatch = useDispatch();

  const eventfilter = (tag, eventsArray) =>
    dispatch(filterEventsByTag(tag, eventsArray));
  const restFilter = (tag, restaurantsArray) =>
    dispatch(filterRestsByTag(tag, restaurantsArray));
  ...
  const handleRemoveTag = (t) => {
    if (rests) {
      let temp = JSON.parse(JSON.stringify(restaurants));
      restFilter(
        restsTags.filter((tag) => tag !== t),
        temp
      );
    } else {
      let temp = JSON.parse(JSON.stringify(events));
      eventfilter(
        eventTags.filter((tag) => tag !== t),
        temp
      );
    }
  };
